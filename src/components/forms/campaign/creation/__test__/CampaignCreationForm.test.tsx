...
describe("Campaign updates", () => {
  ...
  it("submit forms with invalid data", async () => {
    await act(async () => {
      render(
        <AppTheme>
          <div id="main-container">
            <CampaignCreationForm userData={dummyUserData} />
          </div>
        </AppTheme>
      );
    });
    const submitButton = screen.getByText(
      i18n.t("campaign.creation.form.submit")
    );
    fireEvent.click(submitButton);
  });

  it("submit forms with draft data", async () => {
    await act(async () => {
      render(
        <AppTheme>
          <div id="main-container">
    ...
    const titleInput: any = screen.getByTestId(
      "interview.section1.title-placeholder"
    );
    const draftButton = screen.getByText(i18n.t("interview.button.draft"));
    fireEvent.change(titleInput, { target: { value: "hi" } });
    expect(titleInput.value).toBe("hi");
    fireEvent.click(draftButton);
  });

  it("open industry modal", async () => {
    await act(async () => {
      render(
        <AppTheme>
          <div id="main-container">
    ...
    const industryButton = screen.getByText(
      i18n.t("campaign.creation.form.industry-button")
    );
    fireEvent.click(industryButton);
    const industryModalSubmitButton = screen.getByText(
      i18n.t("campaign.creation.modal.submit-button")
    );
    fireEvent.click(industryModalSubmitButton);
  });

  // Add more tests as needed
});

describe("Campaign updates draft second", () => {
  beforeAll(async () => {
    await act(async () => {
      (researchService.getCampaignDetail as any).mockResolvedValue({
        data: dummyData,
...
describe("Campaign submit maximum  draft ", () => {
  beforeAll(async () => {
    await act(async () => {
      (researchService.getCampaignDetail as any).mockResolvedValue({
        data: dummyData,
      ...
      (researchService.getCampaignStatus as any).mockResolvedValue({
        status: 201,
        data: {
          data: { campaignId: "test" },
        },
  ...
  it("submit forms with draft with empty data", async () => {
    render(
      <AppTheme>
        <div id="main-container">
  ...
  it("add questions", async () => {
    ...
    const questionAddButton = screen.getByText(
      i18n.t("campaign.creation.form.question-add")
    );

    fireEvent.click(questionAddButton);
  });

  // Add more tests as needed
});
describe("Campaign submit form with   required data ", () => {
  beforeAll(async () => {
    await act(async () => {
      (researchService.getCampaignDetail as any).mockResolvedValue({
        data: dummyData,
  ...
  it("submit forms with valid data", async () => {
    render(
      <AppTheme>
        <div id="main-container">
          <CampaignCreationForm
    ...
    fireEvent.change(
      screen.getByTestId("interview.section1.title-placeholder"),
      { target: { value: "hi" } }
    );
    fireEvent.change(
      screen.getByTestId("interview.section1.conditions-placeholder"),
      { target: { value: "hi" } }
    );
    ...
    //   }
    // );
    const submit = screen.getByText(i18n.t("campaign.creation.form.submit"));
